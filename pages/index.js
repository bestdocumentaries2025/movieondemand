...
//     const uniqueCategories = Array.from(new Set(movies.map(movie => movie.category))).filter(Boolean)
//     return uniqueCategories
//   }, [movies])

//   // Get the latest movie from each category
//   const latestFromEachCategory = useMemo(() => {
//     const categoryMap = {}
    
//     movies.forEach(movie => {
//       if (!movie.category) return
      
//       // If we haven't seen this category yet, or this movie is newer (based on position in array since it's already reversed)
//       if (!categoryMap[movie.category]) {
//         categoryMap[movie.category] = movie
//       }
//     })
...
//   // Get hero movies (top 5 overall latest)
//   const heroMovies = movies.slice(0, 5)
...
//   // Displayed movies
//   const displayedLatestMovies = latestFromEachCategory.slice(0, latestMoviesLimit)
//   const displayedAllMovies = movies.slice(0, allMoviesLimit)
//   const adultMovies = movies.filter(m => m.category === 'Adult')
//   const displayedAdultMovies = adultMovies.slice(0, adultMoviesLimit)
//   const displayedSearchResults = filteredMovies.slice(0, searchResultsLimit)
...
//                   </h2>
//                   <span className="text-gray-400">
//                     Updated daily • {displayedLatestMovies.length} categories
//                   </span>
//                 </div>
//                 <MovieGrid movies={displayedLatestMovies} title="" />
//                 {latestMoviesLimit < latestFromEachCategory.length && (
//                   <div className="text-center mt-12">
//                     <button
//                       onClick={loadMoreLatest}
//                       className="bg-red-600 hover:bg-red-700 text-white px-10 py-4 rounded-lg font-bold text-lg transition-colors"
//                     >
//                       Load More Latest Movies ({latestFromEachCategory.length - displayedLatestMovies.length} more)
//                     </button>
//                   </div>
//                 )}
//               </div>
