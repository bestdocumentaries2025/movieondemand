...
//       <main className="pt-20 min-h-screen">
//         <div className="container mx-auto px-4 py-8"style={{ marginTop:'150px'}}>
//           {shouldShowVideoPlayer && (
//             <section className="mb-8" aria-labelledby="player-heading">
//               <h2 id="player-heading" className="sr-only">
//                 Video Player for {movie.title}
...
//             <div className="flex flex-col items-center justify-center space-y-4">
//               <div className="w-full max-w-md">
//                 <div className="bg-gradient-to-r from-red-600 to-red-700 rounded-lg p-6 border border-red-500 shadow-lg">
//                   <h3 className="text-xl font-bold mb-3 text-white text-center">Watch Full Movie Now</h3>
//                   <div className="space-y-4">
//                     <p className="text-sm text-white text-center">
//                       Click below to watch the complete movie in high quality
//                     </p>
//                     <Link
//                       href={`/video/${movie.slug}`}
//                       className="flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-red-600 px-6 py-4 rounded-lg transition-all duration-300 hover:scale-105 w-full font-bold text-lg"
//                     >
//                       <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
//                         <path d="M8 5v14l11-7z"/>
//                       </svg>
//                       <span>Play Full Movie Now</span>
//                     </Link>
//                     <p className="text-xs text-red-200 text-center">
//                       Direct streaming in HD quality with no interruptions
//                     </p>
//                   </div>
//                 </div>
//               </div>
...
// // Deterministic functions that return same output on server and client
// const generateSEOTitle = (movie) => {
//   const year = movie.releaseYear || '';
//   const quality = movie.quality ? `${movie.quality} Quality` : 'HD';
//   const genre = movie.genre?.split(',')[0] || 'Movie';
//   const actors = movie.cast?.slice(0, 2).join(', ') || '';
  
//   // Deterministic choice based on movie slug hash
//   const slug = movie.slug || '';
//   const hash = slug.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
//   const index = hash % 5;
  
//   const titles = [
//     `Watch ${movie.title} (${year}) Online FREE in ${quality} - Full Movie HD Streaming`,
//     `${movie.title} (${year}) Full Movie Online Free - Stream ${quality} ${genre} Film`,
//     `Watch ${movie.title} Online Free - ${year} Movie in ${quality} with ${actors}`,
//     `Free Streaming: ${movie.title} (${year}) - Complete ${genre} Movie in ${quality}`,
//     `${movie.title} Movie Online - Watch Free ${year} ${genre} Film in ${quality} HD`
//   ];
  
//   return titles[index] || titles[0];
// }

// const generateSEODescription = (movie) => {
//   const year = movie.releaseYear || '';
//   const quality = movie.quality || 'HD';
//   const genre = movie.genre || 'Movie';
//   const director = movie.director || 'professional director';
//   const actors = movie.cast?.slice(0, 3).join(', ') || 'talented cast';
//   const duration = formatDuration(movie.duration) || 'full-length';
...
//   const descriptions = [
//     `Watch ${movie.title} (${year}) full movie online FREE in ${quality} quality. Stream this ${genre} film directed by ${director}, starring ${actors}. No registration or signup required. Instant HD streaming of ${duration} ${movie.title} complete film with English subtitles. Download ${movie.title} movie free or watch online.`,
//     `Stream ${movie.title} (${year}) online free without ads. Complete ${duration} ${quality} ${genre} movie featuring ${actors}. Directed by ${director}. Watch ${movie.title} full movie HD, 1080p, 4K available. No account needed. Free movie streaming of ${movie.title} with multiple server options.`,
//     `${movie.title} (${year}) - Watch the complete movie online for free in ${quality}. ${genre} film starring ${actors}. ${movie.description || `Experience ${movie.title} in high definition streaming.`} Instant play, no download required. Mobile and desktop compatible.`,
//     `Free HD streaming of ${movie.title} (${year}). Watch online ${quality} ${genre} movie directed by ${director}. Starring ${actors}. ${duration} runtime. No sign up, no payment. Watch ${movie.title} movie now on any device. Multiple mirror links available.`,
//     `Enjoy ${movie.title} (${year}) full movie online free in ${quality}. ${genre} film with ${actors}. ${movie.description || `Stream ${movie.title} complete film without registration.`} Available in multiple qualities: 360p, 480p, 720p, 1080p. Watch now!`
//   ];
  
//   return descriptions[index] || descriptions[0];
// }

// const generateLSIKeywords = (movie) => {
//   const base = [
...
// export default function MoviePage({ movie, seoTitle, seoDescription, lsiKeywords }) {
//   const router = useRouter()
...
//   // Memoized structured data to prevent hydration mismatches
...
//       "datePublished": `${movie.releaseYear}-01-01`,
//     }
...
//     if (movie.genre) {
//       baseSchema.genre = movie.genre.split(', ').map(g => g.trim());
//     }

//     if (movie.duration && movie.duration !== 'N/A') {
//       baseSchema.duration = movie.duration;
//     }
...
//     return baseSchema;
//   }, [movie, seoDescription, currentUrl, absoluteThumbnail]);
...
//   const shareOnTwitter = () => {
//     const shareUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(seoTitle)}`;
//     window.open(shareUrl, '_blank', 'width=600,height=400');
//   }
...
//   // Generate SEO content server-side (deterministic)
//   const seoTitle = generateSEOTitle(movie);
//   const seoDescription = generateSEODescription(movie);
//   const lsiKeywords = generateLSIKeywords(movie);
...
const generateSEOTitle = (movie) => {
  ...
  const titles = [
    `Watch ${movie.title} (${movie.releaseYear || ''}) Online FREE in ${movie.quality || 'HD'} Quality - Full Movie HD Streaming`,
    `${movie.title} (${movie.releaseYear || ''}) Full Movie Online Free - Stream ${movie.quality || 'HD'} ${movie.genre?.split(',')[0] || 'Movie'}`,
    `Watch ${movie.title} Online Free - ${movie.releaseYear || ''} Movie in ${movie.quality || 'HD'} with ${movie.cast?.slice(0, 2).join(', ') || ''}`,
    `Free Streaming: ${movie.title} (${movie.releaseYear || ''}) - Complete ${movie.genre?.split(',')[0] || 'Movie'} in ${movie.quality || 'HD'}`,
    `${movie.title} Movie Online - Watch Free ${movie.releaseYear || ''} ${movie.genre?.split(',')[0] || 'Movie'} in ${movie.quality || 'HD'}`
  ];
...
const generateSEODescription = (movie) => {
  ...
  const descriptions = [
    `Watch ${movie.title} (${movie.releaseYear || ''}) full movie online FREE in ${movie.quality || 'HD'} quality. Stream this ${movie.genre || 'Movie'} film ${movie.director ? `directed by ${movie.director}` : ''}${movie.cast?.length ? `, starring ${movie.cast.slice(0, 3).join(', ')}` : ''}. No registration required. Instant HD streaming of ${formatDuration(movie.duration) || 'full-length'} ${movie.title} complete film.`,
    `Stream ${movie.title} (${movie.releaseYear || ''}) online free without ads. Complete ${formatDuration(movie.duration) || 'movie'} ${movie.quality || 'HD'} ${movie.genre || 'Movie'}${movie.cast?.length ? ` featuring ${movie.cast.slice(0, 3).join(', ')}` : ''}. Watch ${movie.title} full movie HD, 1080p available. No account needed.`,
    `${movie.title} (${movie.releaseYear || ''}) - Watch the complete movie online for free in ${movie.quality || 'HD'}. ${movie.genre || 'Movie'}${movie.description ? ` ${movie.description}` : ''} Instant play, no download required. Mobile and desktop compatible.`,
    `Free HD streaming of ${movie.title} (${movie.releaseYear || ''}). Watch online ${movie.quality || 'HD'} ${movie.genre || 'Movie'}${movie.director ? ` directed by ${movie.director}` : ''}. ${formatDuration(movie.duration) || 'Full movie'} runtime. No sign up, no payment. Watch ${movie.title} movie now.`,
    `Enjoy ${movie.title} (${movie.releaseYear || ''}) full movie online free in ${movie.quality || 'HD'}. ${movie.genre || 'Movie'}${movie.cast?.length ? ` with ${movie.cast.slice(0, 3).join(', ')}` : ''}. Stream ${movie.title} complete film without registration. Available in multiple qualities. Watch now!`
  ];
  
  return descriptions[index] || descriptions[0];
}

export default function MoviePage({ movie, seoTitle, seoDescription }) {
  const router = useRouter()
  ...
  // Generate complete structured data graph
  const generateStructuredData = () => {
      ...
      "@graph": [
        {
          ...
          "articleSection": ["Movie Review", movie.genre?.split(',')[0] || "Entertainment"],
          "inLanguage": "en-US",
          "potentialAction": [
            {
              "@type": "CommentAction",
              "name": "Comment",
              "target": [`${currentUrl}#comments`]
            }
          ]
        },
        {
          ...
          "potentialAction": [
            ...
            {
              "@type": "WatchAction",
              "target": `${SITE_URL}/video/${movie.slug}`
            }
          ]
        },
        {
          ...
          "contentUrl": absoluteThumbnail,
          "width": 800,
          "height": 1200,
          "caption": `${movie.title} - Movie Poster`
        },
        {
          "@type": "BreadcrumbList",
          "@id": `${currentUrl}#breadcrumb`,
        ...
        },
        {
          ...
          "actor": movie.cast && Array.isArray(movie.cast) && movie.cast.length > 0 ? 
            movie.cast.slice(0, 5).map(actor => ({
              "@type": "Person",
              "name": actor
            })) : undefined,
          "genre": movie.genre ? movie.genre.split(', ').map(g => g.trim()) : undefined,
          "duration": movie.duration && movie.duration !== 'N/A' ? movie.duration : undefined,
          "contentRating": movie.rating && movie.rating !== 'N/A' ? movie.rating : undefined,
  ...
  return (
    <div className="min-h-screen bg-black">
      <Head>
        ...
        <meta name="keywords" content={`${movie.title}, ${movie.genre}, ${movie.releaseYear}, watch online, free streaming, movie review`} />
        <link rel="canonical" href={currentUrl} />
...
export async function getStaticProps({ params }) {
  ...
  // Generate SEO content server-side
  const seoTitle = generateSEOTitle(movie);
  const seoDescription = generateSEODescription(movie);
